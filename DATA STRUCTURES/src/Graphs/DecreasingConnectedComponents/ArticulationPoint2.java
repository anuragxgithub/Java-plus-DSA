...
Since the algo Tarjan's Algo so this would be almost similar to the algo which we had written while
...
ignore.
2- If neighor is already visited then we'll simply updat the lowdt of our currrent node which we used to do 
   in bridge finding. (note compare with neighobrs dt[neighbor])
...
4- We also have to maintain the children count of each node so that we can later find out that if the a node
    ...
    public static void dfs(ArrayList<Edge>[] graph, int curr, int parent, int[] dt, int[] lowdt, int time,
        ...
        for(int i=0; i<graph[curr].size(); i++) {
            ...
            if(parent == neighbor) {
                ...
                // update the lowest dt of the curr node
                ...
                if(dt[curr] <= lowdt[neighbor] && parent != -1) {
                    ap[curr] = true;
                }
                children++;
            }
