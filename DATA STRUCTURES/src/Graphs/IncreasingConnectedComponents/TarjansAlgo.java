...
But these are not important thing but the concept which we are going to use in this algo is important in itself.
    ...
    public static void dfs(ArrayList<Edge>[] graph, boolean[] visited, int[] dt, int[] low, int time, int curr, int parent) {
        ...
        for(int i = 0; i < graph[curr].size(); i++) {
            ...
            if(neighbor == parent) {
                ...
                // update the lowest discovery time for current node when returning back
                ...
                // already visited and not parent of current node  (cycle case)
